...
namespace Cactbot
{
    public class CactbotEventSource : EventSourceBase
    {
        ...
        private GameRegion game_region_ = GameRegion.Chinese;
        ...
        public override void Start()
        {
            ...
            pc_locale_ = CultureInfo.CurrentUICulture.Name;
            ...
            if (Config.UserConfigFile != null)
                LogInfo("CactbotUserDirectory: {0}", Config.UserConfigFile);
            ...
            if (int.Parse(net_version[0]) < kRequiredNETVersionMajor || int.Parse(net_version[1]) < kRequiredNETVersionMinor)
                LogError("RequireDotNetVersion: {0}", net_version_str);
        }
        ...
        private Dictionary<string, string> GetLocalUserFiles(string config_dir, string overlay_name)
        {
            ...
            {
                LogError("CheckDirectoryErrorMessage: {0}", e.ToString());
                return null;
            }
            ...
            {
                logger.Log(LogLevel.Error, "PresetRadar", e.ToString());
            }
        ...
        private void GetUserConfigDirAndFiles(string source, string overlay_name, out string config_dir, out Dictionary<string, string> local_files)
        {
            ...
            else
            {
                if (source != null && source != "")
                {
                    ...
                    {
                        logger.Log(LogLevel.Error, "PresetTest", source, e.ToString());
                        config_dir = null;
                        local_files = null;
                    }
                }
                if (local_files == null)
                {
                    ...
                    try
                    {
                        config_dir = Path.GetFullPath(Config.UserConfigFile + "\\user");
                        local_files = GetLocalUserFiles(config_dir, overlay_name);
                    }
                    catch (Exception e)
                    {
                        // Accessing CactbotEventSourceConfig.CactbotDllRelativeUserUri can throw an exception so don't.
                        logger.Log(LogLevel.Error, "PresetXephero", config_dir, e.ToString());
                        config_dir = null;
                        local_files = null;
                    }
                }
        ...
        struct OverlayPreset : IOverlayPreset
        {
            ...
            string IOverlayPreset.HttpUrl => throw new NotImplementedException();
